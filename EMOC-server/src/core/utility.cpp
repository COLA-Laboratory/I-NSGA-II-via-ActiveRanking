...
/*calculate pinv | new zone | hold old zone on*/
    ...
    {
        int size = count(gold_string.begin(),gold_string.end(),',')+1;

        if(!size)
        {
            std::cout<<"\nthe dimensionality of gold point is 0, hence exits."<<std::endl;
            exit(1);
        }

        std::vector<double> gold_point(size);
        int i = 0;

        //std::string::size_type -- unsigned long long
        std::string::size_type start = 0, end;

        while((end = gold_string.find(',',start)) != std::string::npos)
        {
            gold_point[i++] = atof(gold_string.substr(start,end-start).c_str());
            start = end + 1;
        }
        gold_point[i] = atof(gold_string.substr(start).c_str());
