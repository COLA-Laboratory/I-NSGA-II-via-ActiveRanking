...
namespace emoc {
	...
	static void SetParameter(char *parameter_name, char *value, EMOCParameters *para)
	{
        ...
        else if (!strcmp(parameter_name, "g"))
        {
            para->gold_stringType = value;
        }
        ...
        else if (!strcmp(parameter_name, "RANDOM"))
        {
            para->random = (double)atof(value) / 32767.0;
        }
		else
		{
	...
	void PrintObjective(const char *filename, int dec_num, int obj_num, Individual** pop_table, int pop_num, const std::string &problem_name)
	{
		...
		for (int i = 0; i < pop_num; ++i)
		{
            ...
            // TODO: record the decision variables
...
//            }
    ...
    void RecordPop(int run_index, int generation, Global *para)
	{
        ...
        sprintf(output_dir, "/media/gylai/MHD/Desktop/IEMO_LTR/experiments_results");
        mkdir(output_dir,S_IRWXU);

        sprintf(output_dir, "/media/gylai/MHD/Desktop/IEMO_LTR/experiments_results/out");
        mkdir(output_dir,S_IRWXU);

        sprintf(output_dir, "/media/gylai/MHD/Desktop/IEMO_LTR/experiments_results/out/%s",
                algorithm_name.c_str());
        mkdir(output_dir,S_IRWXU);

        sprintf(output_dir, "/media/gylai/MHD/Desktop/IEMO_LTR/experiments_results/out/%s/%s_%dobjs_%ddvs",
                algorithm_name.c_str(),
        ...
        sprintf(output_dir, "/media/gylai/MHD/Desktop/IEMO_LTR/experiments_results/out/%s/%s_%dobjs_%ddvs/%s",
                algorithm_name.c_str(),
                para->problem_name_.c_str(),
                para->obj_num_,
                para->dec_num_,
        ...
        sprintf(output_dir, "/media/gylai/MHD/Desktop/IEMO_LTR/experiments_results/out/%s/%s_%dobjs_%ddvs/%s/%d/",
                algorithm_name.c_str(),
                para->problem_name_.c_str(),
                para->obj_num_,
                para->dec_num_,
    ...
    void RecordPop(int run_index, int generation, Global *para, double *reference_point)
    {
        ...
        sprintf(output_dir_level0, "/media/gylai/MHD/Desktop/IEMO_LTR/experiments_results/out");
        sprintf(output_dir_level1, "/media/gylai/MHD/Desktop/IEMO_LTR/experiments_results/out/%s",
                algorithm_name.c_str()
        );
        sprintf(output_dir_level2, "/media/gylai/MHD/Desktop/IEMO_LTR/experiments_results/out/%s/%s_%dobjs_%ddvs",
                algorithm_name.c_str(),
                para->problem_name_.c_str(),
                para->obj_num_,
                para->dec_num_
        );
        sprintf(output_dir_level3, "/media/gylai/MHD/Desktop/IEMO_LTR/experiments_results/out/%s/%s_%dobjs_%ddvs/%s/",
                algorithm_name.c_str(),
                para->problem_name_.c_str(),
                para->obj_num_,
                para->dec_num_,
        ...
        sprintf(output_dir_level4, "/media/gylai/MHD/Desktop/IEMO_LTR/experiments_results/out/%s/%s_%dobjs_%ddvs/%s/%d/",
                algorithm_name.c_str(),
                para->problem_name_.c_str(),
                para->obj_num_,
                para->dec_num_,
    ...
    void RecordRunTime(std::string testProblem, int Nobj, int Ndec, std::string weight, std::string Algo, double run_time, int run_index)
    {
        char output_dir[MAX_BUFFSIZE];
        char output_file_time[MAX_BUFFSIZE];      // save whole population
        // set the output directory
        std::string problem_name(testProblem);
        ...
        sprintf(output_dir, "../TIME");
        mkdir(output_dir,S_IRWXU);

        sprintf(output_dir, "../TIME/out");
        mkdir(output_dir,S_IRWXU);

        sprintf(output_dir, "../TIME/out/%s_%dobjs_%ddvs",
                testProblem.c_str(),
                Nobj,
                Ndec);
        mkdir(output_dir,S_IRWXU);

        sprintf(output_dir, "../TIME/out/%s_%dobjs_%ddvs/%s",
                testProblem.c_str(),
                Nobj,
                Ndec,
                weight.c_str());
        ...
        sprintf(output_file_time, "%s/%s.txt", output_dir, algorithm_name.c_str());

        FILE *fpt = fopen(output_file_time, "a");
        if(fpt == nullptr)
        {
            std::cerr <<output_file_time<< " doesn't exist." << std::endl;
            std::cerr << "Press enter to exit" << std::endl;
            std::cin.get();
            exit(-1);
        }
        ...
        if (fflush(fpt) == EOF )
        {
            std::cerr <<output_file_time<< " flush error." << std::endl;
            std::cerr << "Press enter to exit" << std::endl;
            std::cin.get();
            exit(-1);
        }
                        ...
                        std::string weight, std::string Algo, int num_cmp, int run_index)
    {
        ...
        sprintf(output_dir, "../NUMCMP");
        mkdir(output_dir,S_IRWXU);

        sprintf(output_dir, "../NUMCMP/out");
        mkdir(output_dir,S_IRWXU);

        sprintf(output_dir, "../NUMCMP/out/%s_%dobjs_%ddvs",
                testProblem.c_str(),
                Nobj,
                Ndec);
        mkdir(output_dir,S_IRWXU);

        sprintf(output_dir, "../NUMCMP/out/%s_%dobjs_%ddvs/%s",
                testProblem.c_str(),
                Nobj,
                Ndec,
                weight.c_str());
