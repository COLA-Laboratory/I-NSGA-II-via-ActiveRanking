...
int main(int argc, char* argv[])
{
    ...
    std::cout << "DM's gold:          " << parameter->gold_stringType << std::endl;
	...
	printf("------total (all sub sum) run time: %fs--------\n", time);

    // 记录程序运行时间
    // emoc::RecordRunTime(parameter->problem_name, parameter->objective_num, parameter->decision_num \
                        ,parameter->weight_stringType, parameter->algorithm_name, time, \
...
void *Work(void *args)
{
    ...
    const char *gold_stringType = parameter->para->gold_stringType.c_str();
	...
	for (int run = parameter->run_start; run <= parameter->run_end; ++run)
	{
        ...
        g_GlobalSettingsArray[thread_id] = new emoc::Global(algorithm_name, problem_name,
                                                            population_num, dec_num, obj_num, max_eval,
                                                            thread_id, output_interval, run, askTime,
                                                            tau, step_size, inquiriesNum, weight_stringType, gold_stringType,
                                                            kappa);
...
void EMOCMultiThreadTest(EMOCParameters *parameter)
{
    ...
    if (parameter->algorithm_name == "nsga2_ltr"
        ...
        // 仅对 3 个使用了 RankNet 的算法进行 Python 多线程预处理工作
        Py_Initialize(); // 初始化
        //判断初始化是否成功
        if(!Py_IsInitialized())
        {
...
void EMOCSingleThreadTest(EMOCParameters *parameter)
{
    ...
    const char *gold_stringType = parameter->gold_stringType.c_str();
    ...
    for (int run = parameter->start_index; run < parameter->start_index + parameter->runs_num; ++run)
	{
        ...
        g_GlobalSettingsArray[thread_id] = new emoc::Global(algorithm_name, problem_name, population_num,
                                                            dec_num, obj_num, max_eval, thread_id,
                                                            output_interval, run, askTime, tau, step_size, inquiriesNum,
                                                            weight_stringType, gold_stringType, kappa);
		g_GlobalSettingsArray[thread_id]->Start();
        end = clock();
		double time = (double)(end - start) / CLOCKS_PER_SEC;
