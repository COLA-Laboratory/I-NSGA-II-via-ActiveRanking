...
beforeTime=`date +%s`
./EMOC -file $config -algorithm meta_nsga2 -W 1,1 -save $save -start_index $start_index -run $run -thread $thread -elicitationInterval $elicitationInterval -inquiriesNum $inquiriesNum -noise $noise >../log/log.$beforeTime \
&&
